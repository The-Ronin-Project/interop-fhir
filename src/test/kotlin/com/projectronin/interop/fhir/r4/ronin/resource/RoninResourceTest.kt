...
class RoninResourceTest {
    @Test
    fun `all subtypes are defined`() {
        // This test is not really running anything (RoninResource is an interface, after all), but it is verifying a critical
        ...
        val jsonSubTypes = RoninResource::class.findAnnotation<JsonSubTypes>()?.value?.map { it.value.jvmName }
            ?: fail { "No JsonSubTypes found" }

        val defaultType =
            RoninResource::class.findAnnotation<JsonTypeInfo>()?.defaultImpl?.jvmName
                ?: fail { "No JsonTypeInfo or default found" }

        val knownSubTypes =
            ClassGraph().acceptPackages("com.projectronin.interop.fhir").enableClassInfo().scan().use {
                it.getClassesImplementing(RoninResource::class.java).standardClasses.map { c -> c.name }
            }
