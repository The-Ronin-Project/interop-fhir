...
abstract class BaseFHIRDeserializer<T : Validatable<T>>(private val clazz: Class<T>) : StdDeserializer<T>(clazz) {
    ...
    private fun readNode(node: JsonNode, parser: JsonParser, fieldName: String, type: KType): Any? {
        ...
        return if (kotlinType == String::class) {
            ...
            val list = node.get(fieldName)?.elements()?.asSequence()?.map { if (it.isNull) null else it.readValueAs(parser, itemType) }?.toList()
                ?: emptyList()
                ...
                if (primitiveDataList.isEmpty()) {
                    ...
                    primitiveDataList.mapIndexed { i, primitiveData ->
                        when (primitiveData) {
                            ...
                            else -> itemType.getDeclaredConstructor(valueType, String::class.java, List::class.java)
            ...
            if (primitiveExtensions == null) {
                ...
                javaType.getDeclaredConstructor(valueType, String::class.java, List::class.java)
