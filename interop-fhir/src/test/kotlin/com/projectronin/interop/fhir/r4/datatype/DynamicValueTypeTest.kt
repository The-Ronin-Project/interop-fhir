...
class DynamicValueTypeTest {
    ...
    private val testDataByType =
        mapOf(
            ...
            DynamicValueType.CODEABLE_CONCEPT to
                TestData(
                    """{"text":"text"}""",
            ...
            DynamicValueType.IDENTIFIER to
                TestData(
                    """{"use":"official"}""",
            ...
            DynamicValueType.PARAMETER_DEFINITION to
                TestData(
                    """{"use":"in","type":"type"}""",
            ...
            DynamicValueType.RANGE to
                TestData(
                    """{"low":{"value":0.0}}""",
                    Range(low = SimpleQuantity(value = Decimal(0.0))),
                ),
            DynamicValueType.RATIO to
                TestData(
            ...
            DynamicValueType.RELATED_ARTIFACT to
                TestData(
                    """{"type":"citation"}""",
                    RelatedArtifact(type = RelatedArtifactType.CITATION.asCode()),
                ),
            DynamicValueType.SAMPLED_DATA to
                TestData(
            ...
            DynamicValueType.TIMING to
                TestData(
                    """{"code":{"text":"code"}}""",
                    Timing(code = CodeableConcept(text = FHIRString("code"))),
                ),
            DynamicValueType.TRIGGER_DEFINITION to
                TestData(
            ...
            DynamicValueType.USAGE_CONTEXT to
                TestData(
