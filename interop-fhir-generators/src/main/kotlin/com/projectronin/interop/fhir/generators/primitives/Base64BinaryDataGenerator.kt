...
class Base64BinaryDataGenerator : DataGenerator<Base64Binary?>() {
    ...
    override fun generateInternal(): Base64Binary = Base64Binary(defaultString)
}

infix fun DataGenerator<Base64Binary?>.of(value: String) = of(Base64Binary(Base64.getEncoder().encodeToString(value.toByteArray())))

infix fun DataGenerator<Base64Binary?>.ofLength(stringLength: Int) {
    val stringValue = Faker().lorem().fixedString(stringLength)
    of(Base64Binary(stringValue))
}
